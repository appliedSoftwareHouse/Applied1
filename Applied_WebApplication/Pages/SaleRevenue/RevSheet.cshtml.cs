...
{
    [Authorize]
    public class RevSheetModel : PageModel
    {
        [BindProperty]
        ...
        public IActionResult OnPostDelete(int ID)
        {
            ...
            if (_Table.Count == 1)
            {
                try
                {
                    ...
                    _Table.Delete();
                    ErrorMessages.Add(SetMessage("Record has been deleted.", Color.Red));
                }
                catch (Exception e)
                {
                    ErrorMessages.Add(SetMessage(e.Message, Color.Red));
                }
            }
            return RedirectToPage("SaveRev");
        }
        public IActionResult OnPostPrint(string Vou_No)
        {
            SetKey(UserName, "cbVouNo", Vou_No, KeyType.Text);
            return RedirectToPage("./SaleRevenue/RevReports", "Voucher", new { _ReportType = ReportType.Preview });
        }
